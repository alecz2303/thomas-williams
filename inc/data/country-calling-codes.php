 ...
 *
 * United States and Mexico intentionally appear first.
 * Remaining entries are alphabetical by display name.
 *
...
/**
 * Return supported international calling codes.
 *
 * @return array<int,array{iso:string,name:string,dial:string}>
 */
function tw_get_country_calling_codes()
{
    return [
...
/**
 * Find a country entry by ISO code.
 *
 * @param string $iso ISO 3166-style two-letter code.
 *
 * @return array|null
 */
function tw_get_country_calling_code_by_iso($iso)
{
    $iso = strtoupper(
        sanitize_text_field($iso)
    );

    foreach (tw_get_country_calling_codes() as $country) {
        if ($country['iso'] === $iso) {
            return $country;
        }
    }
...
/**
 * Generate a flag emoji from a two-letter ISO code.
 *
 * @param string $iso Country code.
 *
 * @return string
 */
function tw_get_country_flag($iso)
{
    $iso = strtoupper($iso);
